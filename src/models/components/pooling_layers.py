...
class Attention1dPoolingProjection(nn.Module):
    def __init__(self, hidden_size, output_dim, dropout=0.25) -> None:
        super(Attention1dPoolingProjection, self).__init__()
        self.linear = nn.Linear(hidden_size, hidden_size)
        self.dropout = nn.Dropout(dropout)
        self.relu = nn.ReLU()
        self.final = nn.Linear(hidden_size, output_dim)
...
class Attention1dPoolingHead(nn.Module):
    ...
    def __init__(
        self, hidden_size: int, output_dim: int, dropout: float = 0.25
        ...
        self.attention1d_projection = Attention1dPoolingProjection(
            hidden_size, output_dim, dropout
        )
...
class MeanPoolingProjection(nn.Module):
    ...
    def __init__(self, hidden_size, output_dim, dropout=0.25):
        super().__init__()
...
class MeanPoolingHead(nn.Module):
    ...
    def __init__(self, hidden_size, output_dim, dropout=0.25):
        ...
        self.mean_pooling_projection = MeanPoolingProjection(
            hidden_size, output_dim, dropout
        )
...
class LightAttentionPoolingHead(nn.Module):
    def __init__(
        ...
        self.feature_convolution = nn.Conv1d(
            hidden_size, hidden_size, kernel_size, stride=1, padding=kernel_size // 2
        )
...
def get_pooler(pooler_type):
    if pooler_type == "mean_pooler":
        return MeanPoolingHead
    elif pooler_type == "light_attention_pooler":
        return LightAttentionPoolingHead
    elif pooler_type == "attention_1d_pooler":
        return Attention1dPoolingHead
    else:
