...
class ONEPROTLitModule(LightningModule):
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor] = None, batch_idx: int = 0, dataloader_idx: int = 0) -> None:
        ...
        batch_data = {}
        modality_id = self.data_modalities[dataloader_idx]
        batch_data[modality_id]= batch
        sequence_outputs, modality_outputs = self.forward(batch_data)

        self.validation_step_outputs.append({modality_id:{'seq':sequence_outputs[modality_id], 'mod':modality_outputs[modality_id]}})
       
        loss = self.loss_fn(sequence_outputs[modality_id], modality_outputs[modality_id])
        self.val_loss(loss)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        #all_validation_out = self.all_gather(self.validation_step_outputs)
        #print("entered")
        ...
        #        all_seqs = torch.stack([x[modality]["seq"] for x in all_validation_out], dim=0)
        #        all_mods = torch.stack([x[modality]["mod"] for x in all_validation_out], dim=0)
           

        #        all_seqs = all_seqs.view(-1, 512)
        #        all_mods = all_mods.view(-1, 512)
        #        print("entered 2 ")
        #        metrics = self.get_clip_metrics(all_seqs, all_mods)
                
        #        for name, value in metrics.items():
        #            print(f'val/{modality}/{name} = {value}')
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor] = None, batch_idx: int = 0, dataloader_idx: int = 0) -> None:
        ...
        self.test_step_outputs.append({modality_id:{'seq':sequence_outputs[modality_id], 'mod':modality_outputs[modality_id]}})
        
        loss = self.loss_fn(sequence_outputs[modality_id], modality_outputs[modality_id])
        
        self.test_loss(loss)
        #self.log('train_loss_ml', nll)
        self.log("test/loss", self.test_loss, on_step=True, on_epoch=True, prog_bar=True, sync_dist=True)


    def on_test_epoch_end(self) -> None:
        "Lightning hook that is called when a validation epoch ends."
        loss = self.test_loss.compute()  # get current val acc
        all_test_out = self.all_gather(self.test_step_outputs)
        
        if self.trainer.is_global_zero:
            
            for modality in self.data_modalities:
              
                all_seqs = torch.stack([x[modality]["seq"] for x in all_test_out], dim=0)
                all_mods = torch.stack([x[modality]["mod"] for x in all_test_out], dim=0)
                ...
                metrics = self.get_clip_metrics(all_seqs.cpu(), all_mods.cpu())
                
                for name, value in metrics.items():
                    print(f'test/{modality}/{name} ==  {value}')
    ...
    def get_clip_metrics(self, sequence_outputs, modality_outputs):
            ...
            print(f"Sequence Shape {sequence_outputs.size()} and Modality Shape {modality_outputs.size()}")
            print(f"Sequence {sequence_outputs[:5,:10]}")
            print(f"Modality {modality_outputs[:5,:10]}")
            ...
            print(f"logits_per_sequence {logits_per_sequence[:10,:10]}")
            print(f"logits_per_modality {logits_per_modality[:10,:10]}")
            
            print(f"max logits_per_sequence {torch.max(logits_per_sequence[0,:])}")
            print(f"min logits_per_sequence {torch.min(logits_per_sequence[0,:])}")
            print(f"mean logits_per_sequence {torch.mean(logits_per_sequence[0,:])}")

            print(f"max logits_per_sequence {torch.max(logits_per_sequence[10,:])}")
            print(f"min logits_per_sequence {torch.min(logits_per_sequence[10,:])}")
            print(f"mean logits_per_sequence {torch.mean(logits_per_sequence[10,:])}")
